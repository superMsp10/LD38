...
public class WorldBuilder : MonoBehaviour
{
    ...
    */
    Dictionary<string, string> hextoColour = new Dictionary<string, string>(){
        {"000000", "Black"},
        {"000080", "Navy Blue"},
        {"0000C8", "Dark Blue"},
        {"0000FF", "Blue"},
    ...
    void Start()
    {
        ...
        {
            spawnPos = Random.insideUnitSphere * planetNum * amplitude;
            GameObject g = (GameObject)Instantiate(planetTypes[Random.Range(0, planetTypes.Length)], spawnPos, Quaternion.identity, planetsStart);
            if (g.tag == "TelePlanet")
            {
                allTeles.Add(g.GetComponentInChildren<Teleporter>());
                planetType = "Teleporting planet";
            }
            else if (g.tag == "HintPlanet")
            {
                planetType = "Hint planet";

            }
            else if (g.tag == "BoostAmount")
            {
                planetType = "Boost amount planet";
            }
            else if (g.tag == "BoostStrength")
            {
                planetType = "Boost strength planet";
            }
            else
            {
            ...
            if (Random.Range(0, 3) == 2)
            {
                ...
                allPeople.Add(per);
                ColorUtility.TryParseHtmlString("#" + new List<string>(hextoColour.Keys)[Random.Range(0, hextoColour.Count)], out per.normal);
                per.mat.material.color = per.normal;
                per.planetType = planetType;

            }

        }

        theif = allPeople[Random.Range(0, allPeople.Count)];
        theif.theif = true;
    ...
    public IEnumerator ShowDialog(float seconds, string message)
    {
        ...
        while (seconds >= 0)
        {
            ...
            dialog.color = Color.Lerp(endColor, originalColor, (seconds / orgSecs));
            seconds -= 1;
